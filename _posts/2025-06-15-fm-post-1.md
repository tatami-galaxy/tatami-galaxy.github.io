...
where $$\delta_{x_1}$$ is the delta measure centered at $$x_1$$. The second expression is meant in a limiting sense, since the delta measure does not have a density. Similarly, we can also compute the conditional velocity field $$u_t(x|x_1)$$. The marginal velocity field $$u_t(x)$$ is then given by :
