...
where $$\phi : R^d \rightarrow R^d$$ is the Optimal Transport map. We don't have access to $$\phi$$. In fact this is what we want to find in the first place. However our current goal is to obtain a ground truth conditional velocity. Given the Optimal Transport formulation, we can attempt to find an optimal $$\psi_t(x|x_1)$$ (and therefore $$u_t(x|x_1)$$) as follows : 
