...
$$\frac{d}{dt}logp_t(\psi_t(x)) = -div(u_t)(\psi_t(x)) \tag{9}$$

This can be derived from (9) and (2).

TODO : derive

Computing $$div(u_t)$$ which is the trace of the Jacobian $$\partial_x u_t(x) \in R^{d \times d}$$ is expensive for commonly used $$d$$ and requires trace estimators like [Hutchinson's trace estimator](https://arxiv.org/abs/1810.01367). Training requires precisely simulating ODE (9) repeatedly. Flow matching provides a much simpler and more efficient framework that does not involve ODE simulations. 
