...
Similarly we can define a conditional velocity field $$u_t(x|x_1)$$. We can impose our diffeomorphism, locally Lipschitz, integrability and mass conservation constraints from above, on the conditional quantities instead. But then we have to verify that given the constraints on the conditionals, the marginals also satisfy them. In particular, we have to prove the following theorem ([Lipman et al.](https://arxiv.org/abs/2412.06264)) : 

**Theorem 2** *(Marginalization) Let $$p_t(x|z)$$ and $$u_t(x|z) be be $$C^1([0, 1) \times R^d)$$. Let $$p_z$$ have bounded support and $$p_t(x) > 0 for all $$x \in R^d$$ and $$t \in [0, 1)$$.
