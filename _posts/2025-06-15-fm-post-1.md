...
$$p_0(x|x_1) = p_0(x), p_1(x|x_1) = \delta_{x_1}(x), \tag{12}$$

where $$\delta_{x_1}$$ is the delta measure centered at $$x_1$$. The second expression is meant in a limiting sense, since the delta measure does not have a density. Similarly, we can also define the marginal velocity field $$u_t(x)$$ in terms of the conditional velocity field $$u_t(x|x_1)$$ : 
