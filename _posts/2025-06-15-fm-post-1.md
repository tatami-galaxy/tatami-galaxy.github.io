...
Given the Optimal Transport formulation, we can attempt to find an optimal $$\psi_t(x|x_1)$$ (and therefore $$u_t(x|x_1)$$) as follows. From (19) and (20) :
...
$$\int_{0}^{1}\int ||u_t(x)||^2p_t(x)dxdt \leq E_{(X_0, X_1) \sim p_0p_1}\int_{0}^{1}||\dot{\psi_t}(X_0|X_1)||^2dt, \tag{22}$$
