...
We also need to prove that $$u_t$$ is integrable in $$t$$ and locally Lipschitz in $$x$$. $$C^1$$ functions are locally Lipschitz and Theorem 2 assumptions imply that $$u_t(x)$$ is $$C^1$$ for all $$x$$. Since $$u_t(x|z)$$ is conditionally integrable : 
