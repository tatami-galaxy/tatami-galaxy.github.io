...
This holds from Leibniz's rule and becasue the assumptions from Theorem 2 make the integrands above integrable.
...
from Leibniz's rule. Multiplying and dividing by $$p_t(x)$$ : 

$$\frac{d}{dt}p_t(x) =  -div_x\int u_t(x|z)\frac{p_t(x|z)p_z(z)}{p_t(x)}p_t(x)dz$$
