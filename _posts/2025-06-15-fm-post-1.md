...
where $$J$$ is the Jacobian. Intuitively, the Jacobian of function represents how it behaves locally as a linear transformation. The Jacobian determinant then gives us a measure of how much the space is getting 'streched' locally. This is the weight on the probability mass that arrived from $$\psi_t^{-1}(x)$$. This also means that in order for us to compute (5), $$\psi_t$$ has to be invertible, and the Jacobian of its inverse must exist. This can be enforced in a principled way in terms of a *diffeomorphism*. 
...
The marginal velocity field $$u_t(x)$$ is then given by : 

$$u_t(x) = \int u_t(x|x_1)p_t(x_1|x)dx_1 \tag{13}$$

TODO : Is this a design choice? Can we weight the conditional $$u_t$$s any other way?
