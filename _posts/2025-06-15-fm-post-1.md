...
where $$\dot{\psi_t} = \frac{d}{dt}\psi_t$$. This will be useful later on. 
...
where $$t \sim U[0,1]$$ (design choice 5). The conditional loss is given by :
...
$$\psi_t^*(x) = t\phi(x) + (1-t)x,  \tag{19}$$

where $$\phi : R^d \rightarrow R^d$$ is the Optimal Transport map.
We don't have access to $$\phi$$.
In fact this is what we want to find in the first place.
However our current goal is to obtain a ground truth conditional velocity.
Given the Optimal Transport formulation, we can attempt to find an optimal $$\psi_t(x|x_1)$$ (and therefore $$u_t(x|x_1)$$) as follows.
We can write equation (13) as : 

$$u_t(x) = E[u_t(X_t|X_1)|X_t = x], \tag{20}$$
...
$$E[f(X,Y)|Y = y] = \int f(x,y)p_{X|Y}(x|y)dx, \tag{21}$$

Now, from (10) :

$$u_t(x|x_1) = \frac{d}{dt}\psi_t(\psi_t^{-1}(x|x_1)|x_1), \tag{22}$$

Therefore we can write $$u_t(X_t|X_1)$$ as :

$$
\begin{aligned}
\u_t(X_t|X_1) & = \int_{0}^{1}E_{X_t} \\
& = 
\end{aligned}
$$
...
$$
\begin{aligned}
\int_{0}^{1}\int ||u_t(x)||^2p_t(x)dxdt & = \int_{0}^{1}E_{X_t} \\
& = 
\end{aligned}
$$
