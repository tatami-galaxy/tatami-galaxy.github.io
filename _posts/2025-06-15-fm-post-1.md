...
To guarantee a solution until $$t = 1$$ for all $$x \in R^d$$, we must make an additional assumption beyond local Lipschitzness. This is the integrability assumption (*design choise 3*) : 
...
In summary, we have defined a velocity field $$u$$ and flow $$\psi$$ which are both constrained to be $$C^r([0,1] \times R^d, R^d)$$ diffeomorphisms. $$u$$ is also required to be locally Lipscitz in $$x \in R^d$$ and intgrable (in the sense of equation 7) in $$t \in [0,1]$$. We have also defined the Continuity Equation (8), which gives us a way to compute $$p_t$$ from $$u_t$$, and the pushforward constrain (4), which allows us to compute $$p_t$$ from $$\psi_t$$. 
