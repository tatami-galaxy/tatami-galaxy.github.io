...
$$u_t(x) = \int _t(x|x_1)p_t(x_1|x)dx_1, \tag{13}$$

where the weights $$p_t(x|x_1)$$ represent the posterior probabilities of $$x1$$ given current sample $$x$$. $$p_t(x|x_1)$$ can be calculated as :

$$p_t(x1|x) = \frac{p_t(x|x_1)p_1(x_1)}{p_t(x)} \tag{14}$$
