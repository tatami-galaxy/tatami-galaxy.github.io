...
The probability path $$p_t$$ can be computed conditional probability paths, where the conditioning is on a single sample. Here we assume we can condition on given target samples $$x_1$$, but we can also condition on source or jointly on both source and target samples, as we will see later. $$p_t$$ is therefore the marginal probability path and is given by : 
