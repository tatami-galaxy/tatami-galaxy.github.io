...
Since we are parameterizing the velocity field, let's call it $$u_t^\theta$$ and the ground truth velocity field $$u_t$$ as before. It can be shown that a family of loss functions known as [Bregman divergences](https://en.wikipedia.org/wiki/Bregman_divergence) allows us to learn $$u_t^\theta (x)$$ in terms of conditional velocities $$u_t(x|z)$$. For now we will use the simplest Bregman divergence which is the Euclidean distance $$D$$. Therefore the true loss is given by : 
