...
So, we have defined a loss function that allows us to learn the parameterized velocity from the conditional velocities $$u_t(\cdot|Z)$$. However we also do not have access to any ground truth $$u_t$$, only target samples. We need to be able to define a valid and meaningful $$u_t(\cdot|Z)$$ and then regress than onto $$u_t^\theta$$. 
