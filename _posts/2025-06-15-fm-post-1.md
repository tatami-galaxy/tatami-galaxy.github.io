...
*Theorem 1* guarantees only the local existence and uniqueness of a $$C^r$$ flow moving each point $$x \in R^d$$ by $$\psi_t(x)$$ in a limited time frame $$t \in [0, t_x)$$.
...
To guarantee a solution until $$t = 1$$ for all $$x \in R^d$$, we must make additional assumptions beyond local Lipschitzness. 
